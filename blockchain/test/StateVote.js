...
describe("LegislativeAssemblyElection", function () {
    ...
    beforeEach(async function () {
        ...
        election = await Election.deploy(await token.getAddress(), voteCost);
        await election.waitForDeployment();
        // Setup election
        await election.addDistrict("Bihar");
        await election.addConstituency("Bihar", "Patna Sahib");
        await election.addConstituency("Bihar", "Patna");
        await election.registerCandidate("Bihar", "Patna Sahib", 101);
        await election.registerCandidate("Bihar", "Patna Sahib", 102);
        await election.registerCandidate("Bihar", "Patna", 103);

        // Approvals
        await token.connect(voter1).approve(await election.getAddress(), voteCost);
        await token.connect(voter2).approve(await election.getAddress(), voteCost);
    });
...
it("returns structured district data", async function () {
    ...
    expect(data.constituencies.length).to.equal(2);
